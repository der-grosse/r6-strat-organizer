...
View (built 2026-07-19): `lib/adaptations.ts` holds pure helpers — `resolveAutoAdaptation` (top/lowest-index adaptation whose filters match; a filter reuses the banned/available + any/all hit logic; multiple filters per adaptation are OR'd), `resolveSelectedAdaptation(strat, selection, bannedOps)`, `applyAdaptationToAssets`. Selection is a string: null/"auto" → auto-resolve, "none" → base strat, or an adaptation id. `StratViewer` takes an `adaptation` prop and merges assets. `StratDisplay` resolves it, renders `AdaptationSelector` (shadcn Select) inline in the name header (only when `!drawingID && adaptations.length>0`); read-only when no `onChange`.
